...
def retrieve_articles_for_category(user_cat,ARTICLE_COUNT,page):
    categories = get_categories(article_collection)
    if user_cat in categories:
        articles = get_successive_articles_for_category(article_collection,user_cat,ARTICLE_COUNT,page)
    else:
        articles = []
